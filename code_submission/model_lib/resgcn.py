...
class ResGCNBlock(torch.nn.Module):
    ...
    def forward(self, input, edge_index, edge_weight):
        return self.model.forward(input, edge_index, edge_weight)
...
class ResGCN(torch.nn.Module):
    ...
    def init_model(self, n_class, features_num):
        ...
        self.res_conv = ResGCNBlock(hidden, hidden, nbaselayer=num_layers, dropout=self.hyperparameters['dropout_rate'])
        self.out_lin = nn.Linear(self.res_conv.get_outdim(), n_class)
    ...
    def forward(self, data):
        ...
        x = self.res_conv(x, edge_index, edge_weight)
        x = self.out_lin(x)
        return x
