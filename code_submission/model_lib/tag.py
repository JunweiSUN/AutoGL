    ...
    def init_model(self, n_class, feature_num):
        ...
        if self.hyperparameters['use_linear']:
            ...
            for i in range(num_layers):
                self.convs.append(TAGConv(in_channels=hidden_size, out_channels=hidden_size, K=K))
            self.output_lin = Linear(hidden_size, n_class)
        else:
            if num_layers == 1:
                self.conv1 = TAGConv(in_channels=feature_num, out_channels=n_class, K=K)
            else:    
                self.conv1 = TAGConv(in_channels=feature_num, out_channels=hidden_size, K=K)
                self.convs = torch.nn.ModuleList()
                for i in range(num_layers - 2):
                    self.convs.append(TAGConv(in_channels=hidden_size, out_channels=hidden_size, K=K))
                self.conv2 = TAGConv(in_channels=hidden_size, out_channels=n_class, K=K)
