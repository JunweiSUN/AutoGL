...
class GraphSAGE(torch.nn.Module):
    ...
    def init_model(self, n_class, feature_num):
        ...
        if self.hyperparameters['use_linear']:
            ...
            if num_layers == 1:
                self.conv1 = SAGEConv(in_channels=feature_num, out_channels=n_class,normalize=True)
            else:
                self.conv1 = SAGEConv(in_channels=feature_num, out_channels=hidden_size,normalize=True)
                self.convs = torch.nn.ModuleList()
                for i in range(num_layers - 2):
                    self.convs.append(SAGEConv(in_channels=hidden_size, out_channels=hidden_size))          
                self.conv2 = SAGEConv(in_channels=hidden_size, out_channels=n_class,normalize=True)
