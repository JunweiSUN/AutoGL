...
class JKGCN(torch.nn.Module):
    ...
    def init_model(self, n_class, features_num):
        ...
        self.jk_conv = DenseGCNBlock(hidden, hidden, nbaselayer=num_layers, dropout=self.hyperparameters['dropout_rate'])
        self.out_lin = nn.Linear(self.jk_conv.get_outdim(), n_class)
    ...
    def forward(self, data):
        ...
        x = self.jk_conv(x, edge_index, edge_weight)
        x = self.out_lin(x)
        return x
